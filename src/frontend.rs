...
impl Front {
    pub fn new() -> Front {
        ...
        let mut inst = Front {
            underlying: Editor::with_config(config),
        };
        inst.underlying.set_helper(Some(h));
        inst.underlying
            .bind_sequence(KeyPress::Meta('N'), Cmd::HistorySearchForward);
        inst.underlying
            .bind_sequence(KeyPress::Meta('P'), Cmd::HistorySearchBackward);
